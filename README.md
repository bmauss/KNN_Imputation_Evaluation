...
With a single neighbor, we have our best results!  Why is it still so low, though?  Well, a lot of it could be attributed to the heavy class imbalance. As was mentioned, the Iris dataset, while small, was balanced and pristinely curated, while the Seattle Terry Stop dataset is a bloated mess with heavy class imbalance not only in the target variable, but also among our categorical data.  For example, in terms of quantity, caucasian subjects/officers far outnumber other races due to the low population of people of color in the Seattle area). Another reason, however, is that there is too much noise in the data.  Remember that the Iris classes were distinct enough that the KNNImputer was able to remain relatively consistent.  This points to the features in that dataset being good predictors of the target variable classes.  Well, the Terry Stops set could be the exact opposite. The features (independent variables) don't have a strong enough relationship to our target (the dependent variable), which makes them poor predictors.  To make a definitive statement on the efficacy of KNN Imputation on categorical data versus continuous features, we would first need to balance out the Terry Stops dataset and remove columns that have little to no relationship to the target variable.  Suffice it to say that KNN Imputation is a powerful technique for dealing with missing data, but should not be used as a one-size-fits-all solution.
